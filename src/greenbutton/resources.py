...
from greenbutton import objects
...
class ReadingType(Resource):
    def __init__(self, entry, meterReadings=[]):
        ...
        self.tou = utils.getEntity(obj, "espi:tou", lambda e: TOUType(int(e.text)))
...
class LocalTimeParameters(Resource):
    def __init__(self, entry, usagePoints=[]):
        ...
        obj = entry.find("./atom:content/espi:LocalTimeParameters", utils.ns)
        self.tzOffset = utils.getEntity(obj, "espi:tzOffset", lambda e: int(e.text))
...
class IntervalBlock(Resource):
    ...
    def __eq__(self, other):
        if not isiutils.nstance(other, IntervalBlock):
            return False
